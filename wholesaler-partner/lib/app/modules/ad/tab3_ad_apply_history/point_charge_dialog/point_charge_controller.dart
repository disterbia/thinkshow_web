...
class PointChargeController extends GetxController {
  ...
  chargeBtnPressed(BuildContext context) async {
    // check if textfield is empty
    if (pointChargeController.text.isEmpty) {
      mSnackbar(message: '충전 포인트를 입력해주세요.',context: context);
      return;
    }
    // check if name is empty
    if (nameController.text.isEmpty) {
      mSnackbar(message: '입금자명 을 입력해주세요.',context: context);
      return;
    }
    ...
    if (pointInt < 1000) {
      mSnackbar(message: '1000원 이상의 포인트를 충전해주세요.',context: context);
      return;
    }
