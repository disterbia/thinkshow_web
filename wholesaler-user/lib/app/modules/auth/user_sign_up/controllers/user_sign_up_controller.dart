...
class SignupOrEditController extends GetxController {
  ...
  Future<void> checkIdAvailableBtnPressed(BuildContext context) async {
    log('entered id is: ' + idController.text);
    if (idController.text == '') {
      mSnackbar(message: 'ID를 입력하세요.',context: context);
      return;
    }
    isIdAvailable =
    ...
    mSnackbar(
        message:
            isIdAvailable ? '사용가능한 ID입니다.' : '사용 불가능한 ID입니다. 다른 ID를 입력하세요.',context: context);
  }
  ...
  Future<void> saveOrEditBtnPressed(BuildContext context) async {
    ...
    if (passwordVerifyController.text == '' && isEditing.isFalse) {
      mSnackbar(message: '비밀번호 확인를 입력하세요.',context: context);
      return;
    }
    if (passwordController.text != passwordVerifyController.text &&
        isEditing.isFalse) {
      mSnackbar(message: '비밀번호를 확인하세요.',context: context);
      return;
    }
    if (passwordController.text.length < 8 && isEditing.isFalse) {
      mSnackbar(message: '비밀번호를 8자 이상으로 입력해주세요.',context: context);
      return;
    }
    ...
    if (address1Controller.text == '' ||
      ...
      mSnackbar(message: '주소를 확인하세요.',context: context);
      return;
    }
    ...
    if (phoneCtr.certifyId == -1 && isEditing.isFalse) {
      mSnackbar(message: '휴대폰 번호를 인증하세요.',context: context);
      return;
    }
    ...
    if (isAgreeToAll.isFalse && isEditing.isFalse) {
      mSnackbar(message: '이용약관 및 개인정보취급방침에 동의해주세요.',context: context);
      return;
    }
    ...
    if (isEditing.isTrue) {
      if (!RegExp(
        ...
        mSnackbar(message: '이메일 주소가 잘못 되었습니다.',context: context);
        return;
      }

      bool isSuccess = await _apiProvider.putUserInfoEdit();
      if (isSuccess) {
        mSnackbar(message: '수정 완료되었습니다.',context: context);
        Get.back();
        return;
      } else {
        mSnackbar(message: '수정 실패하였습니다.',context: context);
        return;
      }
    } else {
      bool isSuccess = await _apiProvider.postUserSignUp();
      if (isSuccess) {
        mSnackbar(message: '회원가입 완료되었습니다.',context: context);
