...
class Cart2PaymentController extends GetxController {
  ...
  // 포인트 상요 button
  usePointBtnPressed(BuildContext context) {
    ...
    if (pointController.text.isNotEmpty) {
      try {
        ...
        if (tempUsedPoints > cart2checkoutModel.value.userInfo!.point.value) {
          mSnackbar(message: '포인트 사용가능 금액을 초과하였습니다.',context: context);
        }
        // check if used point is more that product price
          ...
          mSnackbar(message: '사용하신 포인트가 결제금액을 초과하였습니다.',context: context);
        } else {
        ...
        mSnackbar(message: '숫자만 입력해주세요.',context: context);
        return;
      }
    }
  }
  ...
  /// 결제하기 button
  paymentBtnPressed(BuildContext context) {
    // check if all fields are filled
    if (nameController.text.isEmpty) {
      mSnackbar(message: '이름을 입력해주세요.',context: context);
      return;
    }
    if (address1ZipCodeController.text.isEmpty) {
      mSnackbar(message: '우편번호를 입력해주세요.',context: context);
      return;
    }
    if (address2Controller.text.isEmpty) {
      mSnackbar(message: '주소를 입력해주세요.',context: context);
      return;
    }
    if (address3Controller.text.isEmpty) {
      mSnackbar(message: '상세주소를 입력해주세요.',context: context);
      return;
    }
    if (phoneFirstPartController.text.isEmpty || phoneSecondPartController.text.isEmpty || phoneThirdPartController.text.isEmpty) {
      mSnackbar(message: '전화번호를 입력해주세요.',context: context);
      return;
    }
    context.go(MyRoutes.Payment);
  }

  Future<void> paymentSuccessful(Map<String, dynamic> result,BuildContext context) async {
    // print(
    ...
    if (isSuccess) {
      ...
      mSnackbar(message: '결제가 완료되었습니다.',context: context);
    } else {
      mSnackbar(message: '결제에 실패했습니다.',context: context);
    }
  }
}
