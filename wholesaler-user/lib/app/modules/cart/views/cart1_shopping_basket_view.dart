...
class Cart1ShoppingBasketView extends GetView {
  ...
  Widget _deleteTextButtons(BuildContext context) {
    return Row(
      children: [
        TextButton(
          onPressed: () => ctr.cartItems.isEmpty
              ? mSnackbar(message: "상품이 없습니다.",context: context)
              : ctr.getTotalSelectedProducts() == 0
                  ? mSnackbar(message: "선택된 상품이 없습니다.",context: context)
      ...
      child: ElevatedButton(
        onPressed: () async {
          if (ctr.getTotalSelectedProducts() != 0) {
            await ctr.postOrderCheckout(context);
          } else {
            mSnackbar(message: '상품을 선택해주세요.',context: context);
          }
        },
        child: Text(
          '구매하기',
