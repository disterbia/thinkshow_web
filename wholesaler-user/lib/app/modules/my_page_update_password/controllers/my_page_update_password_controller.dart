...
class MyPageUpdatePasswordController extends GetxController {
  ...
  changePasswordPressed(BuildContext context) async {
    //print('change password pressed');
    // check if empty
    if (originalPasswordCtr.text.isEmpty || newPasswordCtr.text.isEmpty || newPasswordVerifyCtr.text.isEmpty) {
      mSnackbar(message: '모든 항목을 입력해주세요.',context: context);
      return;
    }

    // check if same
    if (newPasswordCtr.text != newPasswordVerifyCtr.text) {
      mSnackbar(message: '새 비밀번호와 새 비밀번호 확인이 일치하지 않습니다.',context: context);
      return;
    }
    ...
    if (isSuccess) {
      mSnackbar(message: '비밀번호 변경 완료 되었습니다.',context: context);
      mFuctions.userLogout(context);
    } else {
      mSnackbar(message: '비밀번호 변경 실패 되었습니다.',context: context);
    }
  }
}
